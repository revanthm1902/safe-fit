          ...
          <Card className="p-6 bg-white/10 backdrop-blur-lg border border-white/20">
            ...
            <div className="h-48 flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={stepsData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} startAngle={90} endAngle={450} dataKey="value">
                    {stepsData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
      ...
      delay: 0.6
        ...
        <Card className="p-6 bg-white/10 backdrop-blur-lg border border-white/20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            label: "Set Reminder",
            color: "from-orange-500 to-red-500"
          }].map((action, index) => <Button key={index} className={`bg-gradient-to-r ${action.color} hover:opacity-90 text-white font-medium`}>
                {action.label}
              </Button>)}
          </div>
        </Card>
