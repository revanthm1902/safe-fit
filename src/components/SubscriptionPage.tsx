...
const SubscriptionPage: React.FC<SubscriptionPageProps> = ({
  ...
  const subscriptionPlans = [{
    ...
    price: '₹999',
    icon: Shield,
    features: ['Health Tracking', 'Basic Safety Features', 'Emergency SOS', 'Community Support'],
    ...
    price: '₹3650',
    icon: Crown,
    features: ['All Basic Features', 'Advanced Fitness Analytics', 'AI Health Assistant', 'Premium Support', 'Family Safety Sharing'],
    recommended: true,
  ...
  return <div className="min-h-screen bg-safefit-white p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          {subscriptionPlans.map((plan, index) => <motion.div key={plan.id} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          y: -5
              ...
              <Card className={`p-6 border ${plan.recommended ? 'border-safefit-highlight ring-2 ring-safefit-highlight/30' : 'border-safefit-border'}`}>
                <div className="relative">
                  {plan.recommended && <span className="absolute -top-4 right-0 bg-safefit-highlight text-white text-xs px-3 py-1 rounded-full">
                      Best Value
                    </span>}
                  ...
                  <ul className="space-y-3 mb-6">
                    {plan.features.map((feature, i) => <li key={i} className="flex items-center text-safefit-primary">
                        <Check size={18} className="mr-2 text-safefit-highlight" />
                        <span className="text-base">{feature}</span>
                      </li>)}
                  </ul>

                  <Button onClick={() => handleSubscribe(plan.id)} disabled={loading} variant="default" className={`w-full ${plan.recommended ? 'bg-safefit-highlight hover:bg-safefit-highlight/90' : 'bg-safefit-primary hover:bg-safefit-primary/90'}`}>
